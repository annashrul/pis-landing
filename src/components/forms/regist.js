...
export default (props) => {
  ...
  const onChangeAlphaNumericInput = (e) => {
    ...
    if (value.match(regex) || value === "") {
      ...
      if (validateUser !== '' && validateUser !== 'Username tersedia.') dispatch(validateUsername({username:value}))
    }
  }
        ...
        <TextColumn textOnLeft={textOnLeft}>
          <TextContent>
            ...
            <Form onSubmit={handleSubmit(onSubmit)}>
              <Input type="text" {...register("nama", { required: "Nama Lengkap tidak boleh kosong." })}  placeholder="Nama Lengkap" />
              <ErrorText>{errors.nama===undefined?'':errors.nama.message}</ErrorText>
              <GroupInput>
                <IcInput>+62</IcInput>
                <Inputs type="text" {...register("mobile_no", { required: "No. Handphone tidak boleh kosong." })} placeholder="No. Handphone" />
              </GroupInput>
              <ErrorText>{errors.mobile_no===undefined?'':errors.mobile_no.message}</ErrorText>
              <HelpText>Gunakan nomor handphone yang terhubung dengan whatsapp.</HelpText>
              <Input 
              {...register("username", {
                required: "Username tidak boleh kosong.", 
                minLength: {value:"6", message:"Password minimal 6 karakter."},
                pattern:{value:/^[0-9a-zA-Z(\-)]+$/,message:"Hanya gunakan angka, huruf dan tanpa spasi!"}
              })} 
              value={valOfUsername}
              onChange={(e=>onChangeAlphaNumericInput(e))}
              onBlur={e => onBlurUsername(e)}
              placeholder="Username"/>
              <ErrorText>{errors.username===undefined?'':errors.username.message}</ErrorText>

              <Input type="password" {...register("password", { required: "Password tidak boleh kosong." })} placeholder="Password" />
              <ErrorText>{errors.password===undefined?'':errors.password.message}</ErrorText>

              <Input 
                type="password" 
                {...register("password_confirm", { required: "Password Konfirmasi tidak boleh kosong."})} 
                placeholder="Konfirmasi Password"
                onChange={(e)=>comparePass(e)}
                />
              <ErrorText>{errors.password_confirm===undefined?'':errors.password_confirm.message}</ErrorText>
              ...
              <Controller
                ...
                />
              <ErrorText>{errors.bank===undefined?'':errors.bank.message}</ErrorText>

              <Input type="text" {...register("acc_name", { required: "Atas nama tidak boleh kosong." })} placeholder="Atas Nama" />
              <ErrorText>{errors.acc_name===undefined?'':errors.acc_name.message}</ErrorText>

              <Input type="text" {...register("acc_no", { required: "No. rekening tidak boleh kosong." })} placeholder="No. Rekening" />
              <ErrorText>{errors.acc_no===undefined?'':errors.acc_no.message}</ErrorText>
